layui.use(['table','layer','laypage'], function(){
    var table = layui.table, $= layui.jquery,layer = layui.layer,laydate = layui.laydate,laypage = layui.laypage;
    ...
    function addUser(auctionPlateNum,name,mobile,depositAmount,index) {
        $.ajax({
            ...
            success : function(data){
                if(data.success){
                    parent.layer.msg('添加成功，在会员管理可查看详情！');
                    parent.layer.close(index)
                }else{
...
//        查询按钮
    $('#search').click(function(){
        count($('.stutas').val(),$('.theme').val(),beginTime,endTime)
...
//        queryList();
    ...
    init();
    //分页
    function count(endTimestatus,title,beginTime,endTime){
        endTimestatus=endTimestatus?endTimestatus:'';
        title=title?title:'';
        beginTime=beginTime?beginTime:'';
        endTime=endTime?endTime:'';
                ...
                if(data.success) {
                    ...
                    laypage.render({
                        ...
                        , jump: function (obj, first) {
                            ...
                            if (!first) {
                                queryList($('.stutas').val(), $('.theme').val(), beginTime, endTime, obj.curr, limit)
                            } else {
                                if (data.success) {
                                    if (data.data.list.length < 1) {
                                        layer.msg('目前没有该条件的场次信息，请添加');
