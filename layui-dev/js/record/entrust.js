...
//        日期范围获取
    ...
    function init() {
        shop()
        count(page,limit,$('#searchName').val(),$('#carSearchName').val(),$('#customerStoreId').val(),$('#carStoreId').val(),beginTime,endTime)
    }
    function count(page,limit,searchName,carSearchName,customerStoreId,carStoreId,beginTime,endTime) {
        var data={
            page:page?page:1,
        ...
        setAjax('/boss/entrust/getCarEntrustRecordList',data,countView,noFn)
    }
        ...
        if(data.success){
            ...
            laypage.render({
                ...
                ,jump: function(obj, first){
                    ...
                    if(!first){
                        queryList(obj.curr,limit,$('#searchName').val(),$('#carSearchName').val(),$('#customerStoreId').val(),$('#carStoreId').val(),beginTime,endTime)
                    }else{
                        tableView(data)
                    }
                }
    ...
    function queryList(page,limit,searchName,carSearchName,customerStoreId,carStoreId,beginTime,endTime) {
        var data={
            page:page?page:1,
            limit:limit?limit:10,
            searchName:searchName?searchName:null,
        ...
        setAjax('/boss/entrust/getCarEntrustRecordList',data,tableView,noFn)
    }
    function tableView(data) {
        ...
        if(data.success){
            ...
            if(myData.list.length<1){
                layer.msg('没有委托价记录,请核对查询信息！')
                $('table tbody').html('<tr><td colspan="11" style="text-align:center;height:200px">没有委托价记录,请核对查询信息！</td></tr>')
            }else{
                $('table tbody').html('')
